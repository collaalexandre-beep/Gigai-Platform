...
import {
  ...
  ShoppingCart, Clock, PackageCheck, CircleX, Filter,
} from "lucide-react";
...
const PURCHASE_STATUS_META: Record<string, { label: string; color: string }> = {
  aguardando_informacoes: { label: "Aguardando info",    color: "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300" },
  aguardando_aprovacao:   { label: "Aguardando aprova.", color: "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400" },
  aprovado:               { label: "Aprovado",           color: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400" },
  em_cotacao:             { label: "Em cotação",         color: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400" },
  comprado:               { label: "Comprado",           color: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400" },
  aguardando_entrega:     { label: "Aguardando entrega", color: "bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400" },
...
const URGENCIA_META: Record<string, { label: string; dot: string }> = {
  muito_urgente: { label: "Muito urgente", dot: "🔴" },
  urgente:       { label: "Urgente",       dot: "🟡" },
  normal:        { label: "Normal",        dot: "🟢" },
};
...
function PurchaseStatusBadge({ status }: { status: string }) {
  const meta = PURCHASE_STATUS_META[status] ?? { label: status, color: "bg-gray-100 text-gray-700" };
  return <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${meta.color}`}>{meta.label}</span>;
}
...
function PurchaseRequestsTab() {
  ...
  const { data: requests = [], isLoading, refetch, isFetching } = useQuery<PurchaseRequest[]>({
    queryKey: ["/api/purchase-requests", filterStatus],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (filterStatus !== "todos") {
        // map "pendentes" to multiple statuses via separate query
        if (filterStatus === "pendentes") {
          const res = await fetch("/api/purchase-requests");
          const all: PurchaseRequest[] = await res.json();
          return all.filter(r => ["aguardando_informacoes","aguardando_aprovacao","aprovado","em_cotacao"].includes(r.status ?? ""));
        }
        params.set("status", filterStatus);
      }
      const res = await fetch(`/api/purchase-requests${params.toString() ? "?" + params : ""}`);
      return res.json();
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const res = await apiRequest("PATCH", `/api/purchase-requests/${id}/status`, { status });
      return res.json();
    },
    onSuccess: () => {
  ...
  const pendingCount = requests.filter(r =>
    ["aguardando_informacoes","aguardando_aprovacao"].includes(r.status ?? "")
  ).length;
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-4 border-b flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="font-semibold text-sm">Solicitações de Compra</h2>
            <p className="text-xs text-muted-foreground">Pedidos via WhatsApp Bot</p>
          </div>
          {pendingCount > 0 && (
            <span className="ml-1 inline-flex items-center justify-center rounded-full bg-yellow-500 text-white text-xs font-bold w-5 h-5">
              {pendingCount}
            </span>
          )}
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => refetch()} data-testid="button-refresh-purchases">
      ...
      <div className="px-6 py-2 border-b bg-muted/20">
        <div className="flex items-center gap-1">
          ...
          {[
            ...
            { value: "aguardando_aprovacao",label: "Aguard. Aprov." },
            ...
            <button
              ...
              data-testid={`filter-purchase-${f.value}`}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!isLoading && requests.length === 0 && (
          <div className="flex flex-col items-center justify-center h-48 text-center">
            <PackageCheck className="w-12 h-12 text-muted-foreground/20 mb-3" />
            <p className="text-sm font-medium text-muted-foreground">Nenhuma solicitação encontrada</p>
            <p className="text-xs text-muted-foreground/60 mt-1">
              Solicitações via WhatsApp aparecerão aqui
            </p>
          </div>
        )}
        {!isLoading && requests.length > 0 && (
          <div className="divide-y">
            {requests.map((req) => {
              ...
              const urg = URGENCIA_META[req.urgencia ?? "normal"] ?? { label: req.urgencia ?? "", dot: "⚪" };
              return (
                <div key={req.id} className="px-6 py-3 hover:bg-muted/30 transition-colors" data-testid={`purchase-row-${req.id}`}>
                  <div className="flex items-start justify-between gap-3">
                    {/* Left: info */}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className="font-mono text-xs font-semibold text-primary" data-testid={`purchase-codigo-${req.id}`}>
                          {formatCodigo(req.codigo)}
                        </span>
                        ...
                        {req.tipoCompra && (
                          <span className="text-xs text-muted-foreground">
                            {TIPO_META[req.tipoCompra] ?? req.tipoCompra}
                          </span>
                        )}
                      </div>
                      <p className="text-sm font-medium truncate">{req.material}</p>
                      <div className="flex items-center gap-3 mt-0.5 text-xs text-muted-foreground flex-wrap">
                        ...
                        {req.osId && (
                          <span>🔖 OS: {req.osId}</span>
                        )}
                        <span className="flex items-center gap-1">
                          <Clock className="w-3 h-3" />
                          {req.createdAt ? formatDistanceToNow(new Date(req.createdAt), { locale: ptBR, addSuffix: true }) : "—"}
                        </span>
                      </div>
                      {isExpanded && req.observacao && (
                        <p className="mt-1.5 text-xs text-muted-foreground bg-muted/40 rounded px-2 py-1">
                          📝 {req.observacao}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                      ...
                      {isExpanded && (
                        <div className="flex flex-col gap-1 items-end">
                          ...
                          >
                            {STATUS_ORDER.map(s => (
                              <option key={s} value={s}>{PURCHASE_STATUS_META[s]?.label ?? s}</option>
                            ))}
                          </select>
                          {req.status !== "cancelado" && (
                            <button
                              onClick={() => updateMutation.mutate({ id: req.id, status: "cancelado" })}
      ...
      {/* Footer count */}
      <div className="px-6 py-2 border-t bg-muted/20 flex items-center justify-between">
        <p className="text-xs text-muted-foreground">{requests.length} solicitação(ões)</p>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b bg-background">
        ...
        <Tabs value={mainTab} onValueChange={setMainTab}>
          <TabsList className="h-7">
            ...
            <TabsTrigger value="compras" className="text-xs h-6 px-3" data-testid="tab-compras">
